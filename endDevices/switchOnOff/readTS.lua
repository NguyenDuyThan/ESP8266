...
--readTS.setChannelID("167742")
--readTS.setKey("SDXH9JAG3OHVL8DL")
--readTS.readData(true, 1, nil) -- args: filename containing writekey, readkey on two separate lines; show debug msg = true; number of results to return; optional callback file to run after completed request
--readTS = nil
--package.loaded["readTS"]=nil
--print(fields[1])
--print(createdTimes[2])

--local moduleName = olala
local M = {}
--_G[moduleName] = M
local readkey
local channelID
...
function M.readData(debug, numResults, callback)
    ...
    if wifi.sta.getip()==nil then 
        print("No connect to modem, please check!")
    end
