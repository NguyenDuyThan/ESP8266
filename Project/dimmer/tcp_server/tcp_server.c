...
void readMsgServer(char *text){
  if(strlen(text)>0){
    memset(msgSer, '\0', MSG_SER_SIZE_MAX);
    sprintf(msgSer,"%s", text);
    if(idxMsgCom ==0){
      idxMsgCom ++;
    }
  }
...
void returnMsgServer(struct espconn *espconn){
  uint8 idx = 0;
  ...
  espconn_send(espconn, msgSerRe, strlen(msgSerRe));
  memset(msgSerRe, '\0', MSG_SER_SIZE_MAX);
  flagSendCbSer = false;
}
...
void TcpServerClienSendCb(void* arg){
  ...
  DBG_PRINT("tcp client ip:%d.%d.%d.%d port:%d\n", tcp_server_local->proto.tcp->remote_ip[0], tcp_server_local->proto.tcp->remote_ip[1], tcp_server_local->proto.tcp->remote_ip[2], tcp_server_local->proto.tcp->remote_ip[3], tcp_server_local->proto.tcp->remote_port);
  returnMsgServer(tcp_server_local);
}
